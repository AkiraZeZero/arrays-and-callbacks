...
// CODE HERE
const totalPopulation = populations.reduce((total, currentElement) => total + currentElement);
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter((pokemonObject) => {if (pokemonObject.CP > 199) {
...
// CODE HERE
let afterTax = orders.map((order) => {
  return order.price * order.tax + order.price
})
...
let bobsTotal = purchases.filter((element) => element.owner === "Bob").reduce((acc, curr) => {
